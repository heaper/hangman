...

{
  (function () {
    ...
    typeof MessageChannel !== 'function') {
      ...
      var requestIdleCallbackBeforeFirstFrame$1 = requestIdleCallbackBeforeFirstFrame  && typeof cancelIdleCallback === 'function';
...
for(var i=0;i<keysA.length;i++){if(!hasOwnProperty$1.call(objB,keysA[i])||!is(objA[keysA[i]],objB[keysA[i]])){return false;}}return true;}var PLUGIN_EVENT_SYSTEM=1;function createResponderListener(responder,props){var eventResponderListener={responder:responder,props:props};{Object.freeze(eventResponderListener);}return eventResponderListener;}var DiscreteEvent=0;var UserBlockingEvent=1;var ContinuousEvent=2;// Intentionally not named imports because Rollup would use dynamic dispatch for
// CommonJS interop named imports.
var UserBlockingPriority$1=Scheduler.unstable_UserBlockingPriority;var runWithPriority$1=Scheduler.unstable_runWithPriority;/**
...
targetInst=null;}{dispatchEventForPluginEventSystem(topLevelType,eventSystemFlags,nativeEvent,targetInst);}}/**
...
return instance;}function getNextHydratable(node){// Skip non-hydratable nodes.
...
if(currentFiber!==null){resumeTimersRecursively(currentFiber);}};function recordEffect(){{effectCountInCurrentCommit++;}}function recordScheduleUpdate(){{if(isCommitting){hasScheduledUpdateInCurrentCommit=true;}if(currentPhase!==null&&currentPhase!=='componentWillMount'&&currentPhase!=='componentWillReceiveProps'){hasScheduledUpdateInCurrentPhase=true;}}}function startRequestCallbackTimer(){{if(supportsUserTiming&&!isWaitingForCallback){isWaitingForCallback=true;beginMark('(Waiting for async callback...)');}}}function stopRequestCallbackTimer(didExpire){{if(supportsUserTiming){isWaitingForCallback=false;var warning=didExpire?'Update expired; will flush synchronously':null;endMark('(Waiting for async callback...)','(Waiting for async callback...)',warning);}}}function startWorkTimer(fiber){{if(!supportsUserTiming||shouldIgnoreFiber(fiber)){return;}// If we pause, this is the fiber to unwind from.
...
{enterDisallowedContextReadInDEV();if(workInProgress.mode&StrictMode){_payload.call(instance,prevState,nextProps);}}var nextState=_payload.call(instance,prevState,nextProps);{exitDisallowedContextReadInDEV();}return nextState;}// State object
...
{enterDisallowedContextReadInDEV();if(workInProgress.mode&StrictMode){_payload2.call(instance,prevState,nextProps);}}partialState=_payload2.call(instance,prevState,nextProps);{exitDisallowedContextReadInDEV();}}else{// Partial state object
...
var returnFiber=hydrationParentFiber;if(returnFiber!==null){switch(returnFiber.tag){case HostRoot:{var parentContainer=returnFiber.stateNode.containerInfo;didNotMatchHydratedContainerTextInstance(parentContainer,textInstance,textContent);break;}case HostComponent:{var parentType=returnFiber.type;var parentProps=returnFiber.memoizedProps;var parentInstance=returnFiber.stateNode;didNotMatchHydratedTextInstance(parentType,parentProps,parentInstance,textInstance,textContent);break;}}}}}return shouldUpdate;}function popToNextHostParent(fiber){var parent=fiber.return;while(parent!==null&&parent.tag!==HostComponent&&parent.tag!==HostRoot&&parent.tag!==DehydratedSuspenseComponent){parent=parent.return;}hydrationParentFiber=parent;}function popHydrationState(fiber){if(fiber!==hydrationParentFiber){// We're deeper than the current hydration context, inside an inserted
// tree.
...
var next=void 0;if(current$$1===null){// no previous state that needs to be preserved.
...
var _nextFallbackChildren=nextProps.fallback;var _primaryChildFragment=createWorkInProgress(currentPrimaryChildFragment,currentPrimaryChildFragment.pendingProps);_primaryChildFragment.return=workInProgress;if((workInProgress.mode&BatchedMode)===NoMode){// Outside of batched mode, we commit the effects from the
...
// normally do.
var _nextPrimaryChildren2=nextProps.children;next=child=reconcileChildFibers(workInProgress,_currentPrimaryChild,_nextPrimaryChildren2,renderExpirationTime);}}workInProgress.stateNode=current$$1.stateNode;}workInProgress.memoizedState=nextState;workInProgress.child=child;return next;}function propagateSuspenseContextChange(workInProgress,firstChild,renderExpirationTime){// Mark any Suspense boundaries with fallbacks as having work to do.
...
stopProfilerTimerIfRunning();}// Check if the children have any pending work.
...
var node=workInProgress.child;while(node!==null){if(node.tag===HostComponent||node.tag===HostText){appendInitialChild(parent,node.stateNode);}else if(node.tag===FundamentalComponent){appendInitialChild(parent,node.stateNode.instance);}else if(node.tag===HostPortal);else if(node.child!==null){node.child.return=node;node=node.child;continue;}if(node===workInProgress){return;}while(node.sibling===null){if(node.return===null||node.return===workInProgress){return;}node=node.return;}node.sibling.return=node.return;node=node.sibling;}};updateHostContainer=function updateHostContainer(workInProgress){// Noop
...
markUpdate(workInProgress);}}else{var _instance5=createInstance(type,newProps,rootContainerInstance,currentHostContext,workInProgress);_appendAllChildren(_instance5,workInProgress,false,false);// (eg DOM renderer supports auto-focus for certain elements).
// Make sure such renderers get scheduled for later work.
if(finalizeInitialChildren(_instance5,type,newProps,rootContainerInstance)){markUpdate(workInProgress);}workInProgress.stateNode=_instance5;}if(workInProgress.ref!==null){// If there is a ref on a host node we need to schedule a callback
...
return workInProgress;}return null;}case DehydratedSuspenseComponent:{return null;}case SuspenseListComponent:{popSuspenseContext(workInProgress);// SuspenseList doesn't actually catch anything. It should've been
...
return;}case Profiler:{{var onRender=finishedWork.memoizedProps.onRender;if(typeof onRender==='function'){{onRender(finishedWork.memoizedProps.id,current$$1===null?'mount':'update',finishedWork.actualDuration,finishedWork.treeBaseDuration,finishedWork.actualStartTime,getCommitTime(),finishedRoot.memoizedInteractions);}}}return;}case SuspenseComponent:case SuspenseListComponent:case IncompleteClassComponent:case FundamentalComponent:return;default:{(function(){{{throw ReactError(Error('This unit of work tag should not have side-effects. This error is likely caused by a bug in React. Please file an issue.'));}}})();}}}function hideOrUnhideAllChildren(finishedWork,isHidden){{// We only have the top Fiber that was inserted but we need to recurse down its
...
{unmountHostComponents(current$$1,renderPriorityLevel);}return;}case FundamentalComponent:}}function commitNestedUnmounts(root,renderPriorityLevel){// While we're inside a removed host node we don't want to call
...
current$$1.return=null;current$$1.child=null;current$$1.memoizedState=null;current$$1.updateQueue=null;current$$1.dependencies=null;var alternate=current$$1.alternate;if(alternate!==null){alternate.return=null;alternate.child=null;alternate.memoizedState=null;alternate.updateQueue=null;alternate.dependencies=null;}}function getHostParentFiber(fiber){var parent=fiber.return;while(parent!==null){if(isHostParent(parent)){return parent;}parent=parent.return;}(function(){{{throw ReactError(Error('Expected to find a host parent. This error is likely caused by a bug in React. Please file an issue.'));}}})();}function isHostParent(fiber){return fiber.tag===HostComponent||fiber.tag===HostRoot||fiber.tag===HostPortal;}function getHostSibling(fiber){// We're going to search forward into the tree until we find a sibling host
...
// node from the tree.
if(currentParentIsContainer){removeChildFromContainer(currentParent,fundamentalNode);}else{removeChild(currentParent,fundamentalNode);}}else if(node.tag===HostPortal){if(node.child!==null){// When we go into a portal, it becomes the parent to remove from.
...
currentParentIsValid=false;}}node.sibling.return=node.return;node=node.sibling;}}function commitDeletion(current$$1,renderPriorityLevel){{// Recursively delete all host nodes from the parent.
// Detach refs and call componentWillUnmount() on the whole subtree.
unmountHostComponents(current$$1,renderPriorityLevel);}detachFiber(current$$1);}function commitWork(current$$1,finishedWork){switch(finishedWork.tag){case FunctionComponent:case ForwardRef:case MemoComponent:case SimpleMemoComponent:{// Note: We currently never use MountMutation, but useLayout uses
// UnmountMutation.
...
cancelTimeout(timeoutHandle);}if(workInProgress!==null){var interruptedWork=workInProgress.return;while(interruptedWork!==null){unwindInterruptedWork(interruptedWork);interruptedWork=interruptedWork.return;}}workInProgressRoot=root;workInProgress=createWorkInProgress(root.current,null);renderExpirationTime=expirationTime;workInProgressRootExitStatus=RootIncomplete;workInProgressRootLatestProcessedExpirationTime=Sync;workInProgressRootLatestSuspenseTimeout=Sync;workInProgressRootCanSuspendUsingConfig=null;workInProgressRootHasPendingPing=false;{spawnedWorkDuringRender=null;}{ReactStrictModeWarnings.discardPendingWarnings();componentsThatTriggeredHighPriSuspend=null;}}function renderRoot(root,expirationTime,isSync){(function(){if(!((executionContext&(RenderContext|CommitContext))===NoContext)){{throw ReactError(Error('Should not already be working.'));}}})();if(expirationTime!==Sync){var didExpire=isSync;stopRequestCallbackTimer(didExpire);}if(root.firstPendingTime<expirationTime){// If there's no work left at this expiration time, exit immediately. This
...
!(IsThisRendererActing.current)){// If we have not processed any new updates during this pass, then this is
...
!(IsThisRendererActing.current)){// We're suspended in a state that should be avoided. We'll try to avoid committing
...
var current$$1=unitOfWork.alternate;startWorkTimer(unitOfWork);setCurrentFiber(unitOfWork);var next=void 0;if((unitOfWork.mode&ProfileMode)!==NoMode){startProfilerTimer(unitOfWork);next=beginWork$$1(current$$1,unitOfWork,renderExpirationTime);stopProfilerTimerIfRunningAndRecordDelta(unitOfWork,true);}else{next=beginWork$$1(current$$1,unitOfWork,renderExpirationTime);}resetCurrentFiber();unitOfWork.memoizedProps=unitOfWork.pendingProps;if(next===null){// If this doesn't spawn new work, complete the current work.
...
var componentName=getComponentName(fiber.type)||'ReactComponent';if(didWarnStateUpdateForUnmountedComponent!==null){if(didWarnStateUpdateForUnmountedComponent.has(componentName)){return;}didWarnStateUpdateForUnmountedComponent.add(componentName);}else{didWarnStateUpdateForUnmountedComponent=new Set([componentName]);}warningWithoutStack$1(false,"Can't perform a React state update on an unmounted component. This "+'is a no-op, but it indicates a memory leak in your application. To '+'fix, cancel all subscriptions and asynchronous tasks in %s.%s',tag===ClassComponent?'the componentWillUnmount method':'a useEffect cleanup function',getStackByFiberInDevAndProd(fiber));}}var beginWork$$1=void 0;{var dummyFiber=null;beginWork$$1=function beginWork$$1(current$$1,unitOfWork,expirationTime){// If a component throws an error, we replay it again in a synchronously
...
// actual time across multiple render passes.
workInProgress.selfBaseDuration=current.selfBaseDuration;workInProgress.treeBaseDuration=current.treeBaseDuration;}}return workInProgress;}function createHostRootFiber(tag){var mode=void 0;if(tag===ConcurrentRoot){mode=ConcurrentMode|BatchedMode|StrictMode;}else if(tag===BatchedRoot){mode=BatchedMode|StrictMode;}else{mode=NoMode;}if(isDevToolsPresent){// Always collect profile timings when DevTools are present.
...
Events:[getInstanceFromNode$1,getNodeFromInstance$1,getFiberCurrentPropsFromNode$1,injection.injectEventPluginsByName,eventNameDispatchConfigs,accumulateTwoPhaseDispatches,accumulateDirectDispatches,enqueueStateRestore,restoreStateIfNeeded,dispatchEvent,runEventsInBatch,flushPassiveEffects,IsThisRendererActing]}};function createRoot(container,options){var functionName='unstable_createRoot';(function(){if(!isValidContainer(container)){{throw ReactError(Error(functionName+'(...): Target container is not a DOM element.'));}}})();warnIfReactDOMContainerInDEV(container);var hydrate=options!=null&&options.hydrate===true;return new ReactRoot(container,hydrate);}function createSyncRoot(container,options){var functionName='unstable_createRoot';(function(){if(!isValidContainer(container)){{throw ReactError(Error(functionName+'(...): Target container is not a DOM element.'));}}})();warnIfReactDOMContainerInDEV(container);var hydrate=options!=null&&options.hydrate===true;return new ReactSyncRoot(container,BatchedRoot,hydrate);}function warnIfReactDOMContainerInDEV(container){{!!container._reactRootContainer?warningWithoutStack$1(false,'You are calling ReactDOM.%s() on a container that was previously '+'passed to ReactDOM.render(). This is not supported.','unstable_createRoot'):void 0;container._reactHasBeenPassedToCreateRootDEV=true;}}var foundDevTools=injectIntoDevTools({findFiberByHostInstance:getClosestInstanceFromNode,bundleType:1,version:ReactVersion,rendererPackageName:'react-dom'});{if(!foundDevTools&&canUseDOM&&window.top===window.self){// If we're in Chrome or Firefox, provide a download link if not installed.
...
var titleLetters = 'Hangman'.split('');

function renderLetter(letter, index) {
  var cssClasses = "title__letter title__letter--".concat(letter.toLowerCase());
  return react.createElement("span", {
    key: index,
    className: "title__letter-wrap"
...
function Title(_ref) {
  ...
  var cssClasses = classnames('title', {
...
function ButtonLink(_ref, ref) {
  ...
  var cssClasses = classnames('button', className, {
    'button--alt': isAlt,
    'cajole': cajole
  });
...
var ButtonLink$1 = react_12(ButtonLink);

var cajoleDelay = 12000;
var titleDelay = 4000;
var buttonDelay = 9000;

function renderStartButton(withIntro, shouldCajole) {
  var cssClasses = classnames('title-screen__button', {
    'title-screen__button--with-intro': withIntro
  });
  return react.createElement(ButtonLink$1, {
    to: "/numPlayers",
    className: cssClasses,
    cajole: shouldCajole
  }, "Start Game");
...
function TitleScreen(_ref) {
  ...
  var [shouldCajole, setShouldCajole] = react_8(false);
  var [playIntro, setPlayIntro] = react_8(withIntro);
  var [isTitleVisible, setIsTitleVisible] = react_8(!withIntro);
  var [isButtonVisible, setIsButtonVisible] = react_8(!withIntro);
  react_9(() => {
    var timers = [createTimer(setShouldCajole, cajoleDelay)];
  ...
  var cssClasses = classnames('main', 'title-screen', {
  ...
  return react.createElement("main", {
    className: cssClasses,
    onClick: skipIntro
  }, react.createElement(Noose, {
    withIntro: playIntro
  }), isTitleVisible ? react.createElement(Title, {
    withIntro: playIntro
  }) : null, isButtonVisible ? renderStartButton(playIntro, shouldCajole) : null);
}
...
function combineReducers(reducer) {
  return function () {
    ...
    for (var i = 0; i < keys.length; i++) {
      var invoke = reducer[keys[i]](state[keys[i]], action);
...
function playersReducer(state, action) {
  switch (action.type) {
    case 'setNumberOfPlayers':
      ...
      for (var i = 0; i < action.numberOfPlayers; i++) {
...
var phrases = ['kansas city royals', 'missouri', 'ireland', 'halloween', 'pumpkin patch'];
...
var defaultPhraseSize = phraseSizes[2];
...
function getPhraseSizeClassName(phrase) {
  if (phrase === null) {
    return defaultPhraseSize.className;
  }

  var longestWordLength = getLongestWordLengthInPhrase(phrase);
  var matchedPhraseSize = phraseSizes.find(phraseSize => {
    return longestWordLength <= phraseSize.maxWordLength && phrase.length <= phraseSize.maxTotalLength;
  });
  return matchedPhraseSize ? matchedPhraseSize.className : defaultPhraseSize.className;
}
function getNumberOfLettersInPhrase(words, endWordIndex) {
  return words.slice(0, endWordIndex).reduce((sum, word) => {
    return sum + word.length;
  }, 0);
}
function getRandomPhrase() {
  var lastPhrase = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
...
var maxIncorrectGuessCount = 5;
function getIncorrectGuessCount(phrase, guessedLetters) {
  if (phrase === null) {
    return 0;
  }
...
function getCorrectGuessCount(phrase, guessedLetters) {
  if (phrase === null) {
    return 0;
  }
...
function isGameLost(phrase, guessedLetters) {
  var incorrectGuessCount = getIncorrectGuessCount(phrase, guessedLetters);
  return incorrectGuessCount > maxIncorrectGuessCount;
}
function isGameWon(phrase, guessedLetters) {
...
function getGameStatus(phrase, guessedLetters) {
  if (phrase === null) {
...
function getGameDuration(startDate, endDate) {
...
function gameReducer(state, action) {
  switch (action.type) {
    ...
    case 'startGame':
      var phrase = action.phrase ? scrubPhrase(action.phrase) : getRandomPhrase(state.phrase);
      return _objectSpread2({}, state, {
    ...
    case 'guessLetter':
      ...
      var letter = action.letter.toLowerCase();

      if (state.guessedLetters.includes(letter)) {
        return state;
      }

      var guessedLetters = [...state.guessedLetters, letter];
      var status = getGameStatus(state.phrase, guessedLetters);
      var endDate = isGameOver(status) ? Date.now() : null;
      return _objectSpread2({}, state, {
        status,
        guessedLetters,
        endDate
...
function renderBackButton(history) {
  return react.createElement("button", {
    ...
    d: "M10 2.5v3.5h7v8h-7v3.5l-7.5-7.5 7.5-7.5z"
  })));
}
...
function SiteHeader(_ref) {
  ...
  var cssClasses = classnames('site-header', {
    'site-header--with-back-button': showBackButton
  });
  return react.createElement("header", {
    className: cssClasses
  }, react.createElement("div", {
    className: "site-header__inner-wrap"
  }, react.createElement("div", {
    className: "logo"
  }, "Hangman"), showBackButton ? renderBackButton(history) : null));
}

function SiteFrame(_ref) {
  var {
    children,
    showBackButton = true
  } = _ref;
...
function renderButton(content, value, setNumPlayers) {
...
function NumberOfPlayersForm(_ref) {
  var {
    numberOfPlayers = 1,
    onSubmit: _onSubmit
  } = _ref;
  var [numPlayers, setNumPlayers] = react_8(numberOfPlayers);
  var firstButtonRef = react_10();
  react_9(() => {
    firstButtonRef.current.focus();
  }, []);
  return react.createElement("form", {
    className: "form",
    onSubmit: e => onFormSubmit(e, numPlayers, _onSubmit)
  }, react.createElement("h1", {
    className: "page-title"
  }, "How Many Players?"), react.createElement(ButtonSet, null, renderButton('1 Player', 1, setNumPlayers, firstButtonRef), renderButton('2 Player', 2, setNumPlayers)));
}

function _onSubmit(numberOfPlayers, dispatch, setIsComplete) {
  dispatch({
...
function NumberOfPlayersScreen(_ref) {
  var {
    numberOfPlayers = 1,
    dispatch
  } = _ref;
  var [isComplete, setIsComplete] = react_8(false);
  return isComplete ? react.createElement(Redirect, {
    push: true,
    to: "/playerName/1"
  }) : react.createElement(SiteFrame, {
    showBackButton: false
  }, react.createElement(NumberOfPlayersForm, {
    numberOfPlayers: numberOfPlayers,
    onSubmit: numPlayers => _onSubmit(numPlayers, dispatch, setIsComplete)
  }));
}
...
function numberOfPlayersRoute(state, dispatch) {
  ...
  if (game.status !== gameStatus.NOT_STARTED) {
    ...
    return react.createElement(NumberOfPlayersScreen, {
...
function TextField(_ref, inputRef) {
  var {
    id,
    name,
    label,
...
var TextField$1 = react_12(TextField);

function onFormSubmit$1(e, name, onSubmit) {
  e.preventDefault();
  onSubmit(name);
}

function PlayerNameForm(_ref) {
  var {
    playerNumber = 1,
    playerName = '',
    onSubmit: _onSubmit
  } = _ref;
  var [name, setName] = react_8(playerName);
  var inputRef = react_10();
  react_9(() => {
    setName(playerName);
  ...
  return react.createElement("form", {
    className: "form",
    onSubmit: e => onFormSubmit$1(e, name, _onSubmit)
  }, react.createElement("h1", {
    className: "page-title"
  }, "Player ", playerNumber), react.createElement(TextField$1, {
    id: "phrase",
    label: "Enter Name:",
    isRequired: true,
    value: name,
...
function getPlayerName(playerNumber, players) {
  var foundPlayer = players.find(player => player.number === playerNumber);
  return foundPlayer ? foundPlayer.name : '';
}

function redirect(numberOfPlayers) {
  var redirectUrl = numberOfPlayers === 1 ? '/game' : '/enterPhrase';
  return react.createElement(Redirect, {
    push: true,
    to: redirectUrl
  });
...
function PlayerNameScreen(_ref) {
  var {
    players = [],
  ...
  var [isComplete, setIsComplete] = react_8(false);
  var [playerNumber, setPlayerNumber] = react_8(initialPlayerNumber);
  react_9(() => {
  ...
  function onSubmit(name) {
    ...
    if (playerNumber === players.length) {
      ...
      history.push("/playerName/".concat(nextPlayerNumber));
      setPlayerNumber(nextPlayerNumber);
    }
  }

  return isComplete ? redirect(players.length) : react.createElement(SiteFrame, null, react.createElement(PlayerNameForm, {
...
function playerNameRoute(_ref, state, dispatch) {
  var {
    history,
    match
  } = _ref;
  ...
  var playerNumber = Number.parseInt(match.params.playerNumber);
  ...
  if (game.status !== gameStatus.NOT_STARTED) {
    ...
    return react.createElement(PlayerNameScreen, {
      playerNumber: playerNumber,
      players: players,
      dispatch: dispatch,
      history: history
...
function TextareaField(_ref, textareaRef) {
  var {
    id,
    name,
    label,
...
var TextareaField$1 = react_12(TextareaField);

function onFormSubmit$2(e, phrase, onSubmit) {
  e.preventDefault();
  onSubmit(phrase);
}

function EnterPhraseForm(_ref) {
  var {
    players = [],
    hostPlayerNumber = 1,
    onSubmit: _onSubmit
  } = _ref;
  var [phrase, setPhrase] = react_8('');
  var textareaRef = react_10();
  react_9(() => {
    textareaRef.current.focus();
  }, []);
  return react.createElement("form", {
    className: "form",
    onSubmit: e => onFormSubmit$2(e, phrase, _onSubmit)
  }, react.createElement("h1", {
    className: "page-title"
  }, getPlayerName(hostPlayerNumber, players)), react.createElement(TextareaField$1, {
    id: "phrase",
...
function _onSubmit$1(phrase, dispatch, setIsComplete) {
  dispatch({
    type: 'startGame',
    phrase
  });
...
function EnterPhraseScreen(_ref) {
  var {
    players = [],
    hostPlayerNumber = 1,
    dispatch
  ...
  return isComplete ? react.createElement(Redirect, {
    push: true,
    to: "/game"
  }) : react.createElement(SiteFrame, null, react.createElement(EnterPhraseForm, {
    players: players,
    hostPlayerNumber: hostPlayerNumber,
    onSubmit: phrase => _onSubmit$1(phrase, dispatch, setIsComplete)
  }));
}
...
function enterPhraseRoute(state, dispatch) {
  ...
  if (game.status !== gameStatus.NOT_STARTED) {
    ...
    return react.createElement(EnterPhraseScreen, {
...
function renderLetter$1(letter, letterIndex, guessedLetters, keySeed) {
  var isLetterGuessed = guessedLetters.includes(letter);
  var cssClass = classnames('letter', {
    'letter--guessed': isLetterGuessed
  });
  var content = isLetterGuessed ? letter : '';
  var animationDelay = isLetterGuessed ? 0 : (letterIndex * .1).toFixed(1);
  var key = keySeed + letterIndex;
  return react.createElement("div", {
    key: key,
    className: cssClass,
    style: {
...
function renderWord(word, wordIndex, words, guessedLetters, keySeed) {
  ...
  var offsetIndex = getNumberOfLettersInPhrase(words, wordIndex);
  return react.createElement("div", {
    className: "word",
    key: wordIndex
  }, letters.map((letter, letterIndex) => renderLetter$1(letter, offsetIndex + letterIndex, guessedLetters, keySeed)));
}

function Phrase(_ref) {
  var {
    phrase = '',
    guessedLetters = [],
    keySeed = 0,
  ...
  var cssClasses = classnames('phrase', getPhraseSizeClassName(phrase), {
...
function _onKeyDown(_ref, setActiveLetter, isDisabled) {
  var {
    key
  } = _ref;

  if (!isDisabled && isLetterKey()) {
    setActiveLetter(key.toLowerCase());
  }
}

function _onKeyUp(_ref2, onSelectLetter, isDisabled) {
  var {
    key
  } = _ref2;
...
function onButtonClick(_ref3, onSelectLetter) {
  var {
    target
  } = _ref3;
...
function Keyboard(_ref4) {
  var {
    guessedLetters = [],
    onSelectLetter,
    isDisabled = false
  } = _ref4;
  var [activeLetter, setActiveLetter] = react_8('');
  var buttonRef = react_10();
  react_9(() => {
    if (buttonRef.current) {
      buttonRef.current.focus();
    }
  }, [guessedLetters]);
  var hasFocusedButton = false;
  return react.createElement("div", {
    className: "keyboard",
    onKeyUp: e => _onKeyUp(e, setActiveLetter, isDisabled),
    onKeyDown: e => _onKeyDown(e, onSelectLetter, isDisabled)
  }, letters.map((letter, index) => {
    var cssClasses = classnames('keyboard__button', {
      'button--active': letter === activeLetter
    });
    var isButtonDisabled = isDisabled || guessedLetters.includes(letter);
    var shouldFocusButton = !isButtonDisabled && !hasFocusedButton;
...
function getBodyPartCssClasses(incorrectGuessCount, threshold) {
...
function renderHead(incorrectGuessCount) {
  var cssClasses = getBodyPartCssClasses(incorrectGuessCount, 1, 'body-head');
  var eyeCssClasses = getBodyPartCssClasses(incorrectGuessCount, 6, 'body-eye');
  var mouthCssClasses = getBodyPartCssClasses(incorrectGuessCount, 6, 'body-mouth');
...
function renderTorso(incorrectGuessCount) {
  var cssClasses = getBodyPartCssClasses(incorrectGuessCount, 2, 'body-torso');
...
function renderLeftArm(incorrectGuessCount) {
  var cssClasses = getBodyPartCssClasses(incorrectGuessCount, 3, 'body-arm', 'body-left-arm');
  return react.createElement("line", {
    className: cssClasses,
    x1: "130",
    y1: "120",
...
function renderRightArm(incorrectGuessCount) {
  var cssClasses = getBodyPartCssClasses(incorrectGuessCount, 4, 'body-arm', 'body-right-arm');
  return react.createElement("line", {
    className: cssClasses,
    x1: "130",
    y1: "120",
...
function renderLeftLeg(incorrectGuessCount) {
  var cssClasses = getBodyPartCssClasses(incorrectGuessCount, 5, 'body-leg', 'body-left-leg');
  return react.createElement("line", {
    className: cssClasses,
    x1: "130",
    y1: "150",
...
function renderRightLeg(incorrectGuessCount) {
  var cssClasses = getBodyPartCssClasses(incorrectGuessCount, 6, 'body-leg', 'body-right-leg');
  return react.createElement("line", {
    className: cssClasses,
    x1: "130",
    y1: "150",
...
function renderBody(incorrectGuessCount) {
  return react.createElement("g", {
    id: "body"
  }, renderHead(incorrectGuessCount), renderTorso(incorrectGuessCount), renderLeftArm(incorrectGuessCount), renderRightArm(incorrectGuessCount), renderLeftLeg(incorrectGuessCount), renderRightLeg(incorrectGuessCount));
}

function Diagram(_ref) {
  var {
    incorrectGuessCount = 0,
    withIntro = false
  } = _ref;
  var cssClass = classnames('diagram', {
    'diagram--with-intro': withIntro
  });
  return react.createElement("svg", {
    className: cssClass,
    viewBox: "0 0 200 270"
  }, renderBody(incorrectGuessCount), react.createElement("g", {
    id: "gallows"
  }, react.createElement("line", {
    className: "gallows-part gallows-base",
    x1: "10",
    y1: "250",
    x2: "90",
    y2: "250"
  }), react.createElement("line", {
    className: "gallows-part gallows-pole",
    x1: "50",
    y1: "250",
    x2: "50",
    y2: "20"
  }), react.createElement("line", {
    className: "gallows-part gallows-support",
    x1: "50",
    y1: "40",
    x2: "70",
    y2: "20"
  }), react.createElement("line", {
    className: "gallows-part gallows-arm",
    x1: "50",
    y1: "20",
    x2: "130",
    y2: "20"
  }), react.createElement("line", {
    className: "gallows-part gallows-rope",
    x1: "130",
    y1: "20",
    x2: "130",
    y2: "60"
...
function formatDuration(duration) {
  ...
  return "".concat(padNumber(hours), ":").concat(padNumber(minutes), ":").concat(padNumber(seconds));
}
...
function Results(_ref) {
  ...
  var playAgainButtonRef = react_10();
  react_9(() => {
    playAgainButtonRef.current.focus();
  }, []);
  return react.createElement("div", {
    className: "game-results"
  }, react.createElement(DataSet, null, react.createElement(DataItem, {
    label: "Correct Guesses:",
    value: correctGuessCount
  }), react.createElement(DataItem, {
    label: "Incorrect Guesses:",
    value: incorrectGuessCount
  }), react.createElement(DataItem, {
    label: "Duration:",
    value: formatDuration(duration)
  })), react.createElement(ButtonSet, null, react.createElement(Button$1, {
    ref: playAgainButtonRef,
    onClick: onPlayAgain
  }, "Play Again"), react.createElement(Button$1, {
    isAlt: true,
    onClick: onQuit
  }, "Quit")));
}
...
function renderTitle(guesserPlayerName, status) {
  switch (status) {
    case gameStatus.WON:
      return "".concat(guesserPlayerName, " Wins!");

    case gameStatus.LOST:
      return "".concat(guesserPlayerName, " Loses!");
...
function _onSelectLetter(letter, status, dispatch) {
...
function _onPlayAgain(players, dispatch) {
  if (players.length === 1) {
    dispatch({
      type: 'startGame'
    });
...
function GameScreen(_ref) {
  ...
  var incorrectGuessCount = getIncorrectGuessCount(phrase, guessedLetters);
  var correctGuessCount = getCorrectGuessCount(phrase, guessedLetters);
  var guesserPlayerName = getPlayerName(guesserPlayerNumber, players);
  var [keySeed, setKeySeed] = react_8(0);
  react_9(() => {
    setKeySeed(Date.now());
  }, [phrase]);
  return react.createElement(SiteFrame, null, react.createElement("h1", {
    className: "page-title"
  }, renderTitle(guesserPlayerName, status)), react.createElement("div", {
    className: "game"
    ...
    keySeed: keySeed,
    withIntro: withIntro
  }), status === gameStatus.STARTED ? react.createElement(Keyboard, {
    guessedLetters: guessedLetters,
    isDisabled: status !== gameStatus.STARTED,
    onSelectLetter: letter => _onSelectLetter(letter, status, dispatch)
  }) : react.createElement(Results, {
    correctGuessCount: correctGuessCount,
    incorrectGuessCount: incorrectGuessCount,
    duration: getGameDuration(startDate, endDate),
    onPlayAgain: () => _onPlayAgain(players, dispatch),
    onQuit: () => _onQuit(dispatch)
  })));
}
...
function gameRoute(state, dispatch) {
  ...
  return game.status === gameStatus.NOT_STARTED ? renderRedirect(players) : react.createElement(GameScreen, _extends({
...
function App() {
  var [state, dispatch] = react_11(rootReducer, initialState);
  console.log(state);
  return react.createElement(BrowserRouter, null, react.createElement(Switch, null, react.createElement(Route, {
    exact: true,
    path: "/",
    render: props => react.createElement(TitleScreen, _extends({}, props, {
      withIntro: true
    }))
...
function hangman() {
  var containerEl = document.querySelector('#app');
  reactDom.render(react.createElement(App, null), containerEl);
}
